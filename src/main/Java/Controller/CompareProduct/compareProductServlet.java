...
public class compareProductServlet extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        int Reportloi;
        if (request.getParameter("productId2").equals("null")) {
            Reportloi = 1;
            request.setAttribute("loi", Reportloi);
            request.setAttribute("productId", productId);
            response.sendRedirect("dataToHomeFromDetail?productId=" + productId + "&loi=" + Reportloi + "");
            System.out.println("da repon");
        } else {
